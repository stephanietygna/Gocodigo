...
func main() {
	...
	for a != "-1" {
		...
		if a != "-1" {
			...
			fmt.Println("Escreva a Distancia")
			n, err = fmt.Scanln(&b)
			if err != nil {
				log.Fatal(err)
			}
			fmt.Println("Escreva o combustivel")
			n, err = fmt.Scanln(&c)
			if err != nil {
				log.Fatal(err)
			}
			...
			insertTrajeto(contract, a, b, c)
		}
	}

	//insertTrajeto(contract, "1", "1000.0", "10.0")
...
func getAllAssets(contract *client.Contract) {
	...
	evaluateResult, err := contract.EvaluateTransaction("GetAllTrajetos")
	if err != nil {
...
func insertTrajeto(contract *client.Contract, id string, Distancia string, Combustivel string) {
	fmt.Printf("\n--> Submit Transaction: InitLedger, function creates the initial set of assets on the ledger \n")

	_, err := contract.SubmitTransaction("CreateTrajeto", id, Distancia, Combustivel)
	if err != nil {
		panic(fmt.Errorf("failed to submit transaction: %w", err))
	}
